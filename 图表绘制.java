...
public class HousePriceDraw {
	...
	public static void main(String[] args){
		...
		frame.add(new HousePriceDraw("D:/Test/houseprice.txt").getChartPanel());
	...
	private static XYDataset createDataset(String folder) {  
        ...
        for(int i=0;i<Pois.size();i++){
        	...
        	String date=Tool.getStrByKey(poi, "<TIME>", "</TIME>", "</TIME>");
        	int[] Date=new int[2];
        	Date=DateTransfer.getDate(date);
        	int month=Date[0];
        	int day=Date[1];
        	timeseries.addOrUpdate(new Day(day,month,2015), Price);
        }
